...
// The reason why we have functions and variables outside the component is to avoid hitting out of memory errors
...
function drawRoad(ctx, collectionOfPoints){
    ctx.beginPath();
...
function drawVehicle(ctx, vPos, compass, imageMap){
...
export const CrossRoadCanvas = ({roads, trafficPosData}) => {
    ...
    useEffect(() => {
        ...
        collectionOfPoints.forEach(r => {drawRoad(ctx, r); drawTrafficLight(ctx,r[0], imageMap)})
        for (let d of trafficPosData){
            const road_id = d.road_id;
            const dataForRoad = d.data;
            for (let roadObj of dataForRoad){
                drawVehicle(ctx, roadObj, roadIdToRoad[road_id].incoming_compass, imageMap)
            }
        }
      }, [trafficPosData]);
    return(
